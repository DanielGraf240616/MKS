...
void sct_led(uint32_t value)
{
	...
	{
		uint8_t sdi = value & 0x01; //Change SDI bits & shift

		HAL_GPIO_WritePin(SCT_SDI_GPIO_Port, SCT_SDI_Pin, sdi);

		value >>= 1;

		HAL_GPIO_WritePin(SCT_CLK_GPIO_Port, SCT_CLK_Pin,1); //Clock
		HAL_GPIO_WritePin(SCT_CLK_GPIO_Port, SCT_CLK_Pin,0);

	}
	HAL_GPIO_WritePin(SCT_NLA_GPIO_Port, SCT_NLA_Pin, 1); //Write
